...
Full suite (final run, finding #17): 4377 passed / 1 environment-only failure (`build-std`) / 28 ignored. The 20-repo `.cargo-build-test.sh` script: 18/20 repos pass; remaining failures are environmental (`zellij`: upstream `include_bytes!` quirk; `tauri`: missing system `javascriptcore`/`webkit2gtk` dev libraries). See finding #17 for the rmeta-checksum wall-clock notes.
