...
use crate::core::compiler::locking::{CompilationLockRef, LockKey, LockingMode, SharedLockType};
...
fn rustc(
    ...
    return Ok(Work::new(move |state| {
        if let Some(lock) = &mut lock {
            state.lock(lock, dependency_locking_mode).unwrap();
        }
            ...
            .exec(
                ...
                &mut |line| {
                    on_stderr_line(
                        ...
                        &target,
                        &mut output_options,
                        None, // TODO: FIX THIS SO PIPEDLINED BUILDS WORK
                    )
                },
            )
            .map_err(|e| {
        ...
        if let Some(lock) = &lock {
            if target.is_custom_build() {
                ...
                state.lock_manager.read_as_dependency(&lock)?;
            }
        }

        println!("finished {:?} {:?}", fingerprint_dir, target.proc_macro());
...
fn rustdoc(build_runner: &mut BuildRunner<'_, '_>, unit: &Unit) -> CargoResult<Work> {
    ...
    Ok(Work::new(move |state| {
        if let Some(lock) = &mut lock {
            state.lock_manager.start_compiling(lock)?;
        }
...
fn on_stderr_line(
    ...
    if on_stderr_line_inner(state, line, package_id, manifest, target, options, lock_key)? {
