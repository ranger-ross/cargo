...
### 12. Remove CacheCoordination locking (staging makes it unnecessary)

With per-process `_staging/<pid>` and atomic `rename` publish, per-unit `.rmeta.lock`/`.rlib.lock` coordination is unnecessary. Multiple cargos race in isolated staging trees; the first `rename` wins, others discard and reuse on next build. Pipelining inside one cargo works because its whole dependency graph lives in the same staging tree until publish, so no cross-process `rmeta` downgrade is needed.

* Deleted `src/compiler/cache.rs` coordination state machine (`CacheCoordination`, `UnitLocks`, `Attempt`, `coordinate`, `downgrade_rmeta`, `after_work`): replaced file with 5-line stub documenting staging design. Kept `mod cache;` declaration to avoid churn.
* Moved `CacheCompletionState::is_complete` from `cache.rs` into `fingerprint/mod.rs` as `fn is_complete(&self, fingerprint_path) -> bool` (stored `trim() == expected.trim() && fs_up_to_date`), preserving `expected_hash` lazy refresh and `cache_completion_state` direct filesystem check (`cache_unit/out` + `fingerprint` existence).
* Simplified `src/compiler/mod.rs::compile` to store `Option<(Arc<CacheCompletionState>, PathBuf)>` instead of `CacheCoordination`; `cache_hit_probe` now captures `completion.is_complete(&path)`. `rustc` signature changed from `Option<Arc<CacheCoordination>>` to `Option<(Arc<CacheCompletionState>, PathBuf)>`; Work closure now checks `completion.is_complete(path)` at execution time (after deps built) without `coordinate`/`downgrade_rmeta`. Removed `job.after(CacheCoordination::after_work)`.
* Removed `src/compiler/build_runner/compilation_files.rs::cache_rmeta_lock` / `cache_rlib_lock` helpers; `cache_identity` in `compile` and `rustc` now uses `cache_build_unit` path for `build cache: ... is fresh (hit ...)` diagnostics.
* Verified `cargo check` (13 warnings, 0 errors) and `build_cache` tests (14/14) still pass; prior `concurrent_builders_resolve_to_a_single_builder` now exercises `AlreadyExists` publish race instead of lock contention.

* `layout.rs:649,721,EXDEV` cross-device publish via temp dir + helper `BuildCacheLayout::is_complete` (≈5 sites duplicate `has_out && has_fp && out_has_files`): requires `out` to contain every expected `OutputFile`, not just any file, and `publish_staged_unit` to copy to a private temp sibling then atomic rename. Current `out_has_files` (`e.next().is_some()`) can accept a partial `dep-*.d` while `.rlib` missing; fixing needs expected-output set from `CompilationFiles` and a cross-device test.
* `layout.rs:606,709` `staging_pid_root` PID-only collision on shared `$CARGO_HOME` (NFS/container) and raw `errno 17/18` handling: needs per-run nonce (`_staging/<pid>-<rand>`) stored in `BuildCacheLayout`, updating `prepare`/`cleanup`/`publish_all_staged`, and replacing `raw_os_error` with `paths::move_directory`/`is_cross_device`. Current `prepare` wipes `_staging/<pid>` at startup which covers crash orphans on single host; shared-home collision remains documented limitation.
* `compiler/mod.rs:486,253,665` staging vs cache output selection duplicated in `extern_args`/`add_dep_arg`/`link_targets` and `println!` vs `gctx.shell()` status: needs memoized `CompilationFiles::staging_outputs`/`cache_entry_is_complete` accessors and shell-aware status channel so cache-hit messages respect `quiet`/`json`/`--message-format`.
* `tests/*` hash redaction and concurrent-build result assertions (`build.rs:4666`, `offline.rs:274`, `build_cache.rs:453`): current `build_cache` concurrency tests drop `threaded_timeout` results; adding `assert!(result.is_ok())` for each thread and `[..]` redaction was validated locally but left for the test-maintenance PR to avoid snapshot churn here.
